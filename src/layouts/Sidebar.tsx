...
export function Sidebar() {
    ...
    px-2 ${isLargeOpen ? "lg:flex" : "lg:hidden"} ${isSmallOpen ? "flex z-[99] bg-white max-h-screen" : "hidden"}`}>
       ...
       <LargeSidebarSection visibleItemCount={5} >
            ...
            <LargeSidebarItem Icon={History} title="Shorts" url="/shorts" />
       ...
       <hr />
        <LargeSidebarSection title="Subscriptions">
            {subscriptions.map(subscription => (
                <LargeSidebarItem Icon={subscription.imgUrl} key={subscription.id} title={subscription.channelName} url={`/@${subscription.id}`} />
            ))}
        </LargeSidebarSection>
...
visibleItemCount = Number.POSITIVE_INFINITY, } : LargeSidebarSectionProps) {
    ...
    const showExpandButton = isExpanded ? childrenArray : childrenArray.length > visibleItemCount
    ...
    return <div>
        ...
        {visibleChildren}
        {showExpandButton && 
            <Button onClick={() => setIsExpanded(!isExpanded)}variant="ghost" className="w-full flex items-center rounded-lg gap-4 p-3">
...
function LargeSidebarItem({ Icon, title, url, isActive = 
false } : LargeSidebarItemProps) {
